...
export default function ModifyShift() {
    ...
    // parse into k-v store on render
    ...
    // state variables for managing DTO state
    ...
    // create date (without time) from date string
    function createLocalDate(dateString) { 
        const [year, month, day] = dateString.split("-").map(Number); 
        return new Date(year, month - 1, day, 0, 0, 0); 
    } 

    // format date for local time for backend
    ...
    // create date (with time) from date string
    ...
    // clockin, clockout, and date are different things to the client. but on the backend, clockin and clockout are
    // both LocalDateTimes. so the backend expects a DTO containing that.
    ...
    // add a new metric to the metric list
    const addNewMetric = () => {
    ...
    // async function to resubmit an edited shift
    ...
    // async function to delete a shift by id
    ...
    // styles imported here for theme access
    // generally not efficient to do this because it creates these styles on renders AND re-renders
